...
send_strata_cox_b2 <- function(params, data) {
  ...
  write_time <- proc.time()[3] - write_time
  params <- add_to_log(params, "send_strata_cox_b2", 0, 0, write_time, write_size)
  return(params)
}
...
prepare_blocks_cox_a2 <- function(params, blocksize) {
  ...
  write_time <- proc.time()[3] - write_time
  params <- add_to_log(params, "prepare_blocks_cox_a2", 0, 0, write_time, write_size)
  return(params)
}
...
finalize_params_cox_b2 <- function(params, data) {
  ...
  params <- add_to_log(params, "finalize_params_cox_b2", read_time, read_size, 0, 0)
  return(params)
}
...
prepare_blocks_cox_b2 <- function(params) {
  ...
  params <- add_to_log(params, "prepare_blocks_cox_b2", read_time, read_size, 0, 0)
  return(params)
}
...
check_colinearity_cox_a2 <- function(params, data) {
  ...
  indicies <- indicies[-(1:length(data$survival$strata))] - num_strata # Get rid of the strata indicators
  ...
  params <- add_to_log(params, "check_colinearity_cox_a2", read_time, read_size, write_time, write_size)
  return(params)
}
...
compute_log_likelihood_cox_b2 <- function(params, data) {
  ...
  pbar <- make_progress_bar_1(params$blocks$num_blocks, "(I-z*z')WX", params$verbose)
  container_ct_z <- 0
  container_ct_cox <- 0
  ...
  for (i in 1:params$blocks$num_blocks) {
    ...
    IZ.tZ.w_xbtemp = w_xb[strt:stp, , drop = FALSE] - z %*% (t(z) %*% w_xb[strt:stp, , drop = FALSE])
  ...
  params$deltal = deltal
  params$tXB.w_xb   = t(data$x) %*% w_xb
  params$tXB.deltal = t(data$x) %*% deltal
  if (params$alg_iteration_counter == 1) {
    params$nullScore = params$tXB.deltal
  }

  tXB.w_xb = params$tXB.w_xb
  write_time <- write_time - proc.time()[3]
  save(tXB.w_xb, file = file.path(params$write_path, "tXB_w_XB.rdata"))
...
compute_inverse_cox_a2 <- function(params, data) {
  ...
  read_time <- read_time - proc.time()[3]
  load(file.path(params$read_path, "tXB_w_XB.rdata")) # load tXB.w_xb
  ...
  for (i in 1:params$blocks$num_blocks) {
    ...
    read_time <- read_time - proc.time()[3]
    IZ.tZ.w_xb = matrix(readBin(con = to_read, what = numeric(), n = n2 * p2,
                                endian = "little"), nrow = n2, ncol = p2)
    read_time <- read_time + proc.time()[3]

    tXA.w_xb = tXA.w_xb + t(data$x[strt:stp, ]) %*% IZ.tZ.w_xb
    if ((i + 1) %in% params$container$filebreak.Cox || i == params$blocks$num_blocks) {
      close(to_read)
      read_size <- read_size + file.size(file.path(params$read_path, filename))
    }
  ...
  m = rbind(cbind(params$tXA.w_xa, tXA.w_xb),
            cbind(t(tXA.w_xb), tXB.w_xb))
...
compute_beta_cox_b2 <- function(params, data) {
  ...
  if (params$step_size < 1) {  # Is this in the wrong spot?
    params$betasB = params$betasBold + (params$betasB - params$betasBold) * params$step_size
  }
  ...
  params$xb_betas_b = data$x %*% params$betasB
...
compute_results_cox_a2 <- function(params, data) {
  ...
  idx                <- c(idxA, idxB + length(params$a_col_names_old))
  ...
  stats$df           <- params$p
  stats$score        <- t(params$nullScore) %*%
...
compute_cox_b2 <- function(params, data) {
  ...
  while (params$alg_iteration_counter <= params$max_iterations && !params$converged) {
